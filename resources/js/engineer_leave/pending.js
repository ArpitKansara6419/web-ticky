...
var pendingDatatable = $("#" + pendingTable).DataTable({
    processing: true,
    serverSide: true,
    searching: false,
    lengthChange: false,
    ...
    ajax: {
        ...
        data: function (d) {
            ...
            d["leave_approve_status"] = 'pending';
        },
    },
    columns: [
        // {
...
$(document).on('click', '.accept_btn',function() {
    let confirmAction = confirm("Are you sure you want to approve this leave?");
    if (confirmAction) {
        $.ajax({
            url: $(this).data('href'),
            type: 'POST',
            ...
            success: function(response) {
                ...
                var currentPage = $("#" + pendingTable).DataTable().page.info().page;
                pendingDatatable.page(currentPage).draw(false);
            }
        });
    }
});
$(document).on('click', '.reject_btn',function() {
    let confirmAction = confirm("Are you sure you want to reject this leave?");
    if (confirmAction) {
        $.ajax({
            url: $(this).data('href'),
            type: 'POST',
